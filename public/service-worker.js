...
const createCacheKey = function (
  ...
  if (
    ...
    url.search += `${
      (url.search ? '&' : '') + encodeURIComponent(paramName)
...
self.addEventListener('push', (event) => {
  ...
  event.waitUntil(
    self.registration.showNotification(notification.title, {
      ...
      icon: 'https://unchained.shop/_next/static/media/logo.41b5589a.svg',
      data: {
...
*/ !(function (e) {
  ...
  return (function e(t, n, r) {
    ...
    for (
      var i = typeof require === 'function' && require, c = 0;
      c < r.length;
      c++
    )
      o(r[c]);
      ...
      1: [
        function (e, t, n) {
          ...
          function s(e, t, n) {
            ...
            return (
              r(
                `Updating LRU order for ${o}. Max entries is ${c}, max age is ${i}`,
              ),
              g
                .getDb(s)
                .then(function (e) {
                ...
                const u = function (e) {
                  a.push(e.toString()),
                    s
                      ? c(
                          new Error(
                            `Both cache and network failed: "${a.join(
                              '", "',
                            )}"`,
                          ),
                        )
                  ...
                  if (v(l)) {
                    ...
                    for (let d = 0; d < l.length; d++) {
                      if (((p = u(l[d])), !t[f].test(p)))
                        throw new TypeError(
                          `Expected all "${h.name}" to match "${
                            h.pattern
                          }", but received \`${JSON.stringify(p)}\``,
                        );
                    ...
                    if (((p = h.asterisk ? c(l) : u(l)), !t[f].test(p)))
                      throw new TypeError(
                        `Expected "${h.name}" to match "${h.pattern}", but received "${p}"`,
                      );
                    o += h.prefix + p;
                  }
                } else o += h;
