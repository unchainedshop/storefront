    ...
    <div className="py-6">
      <div className="lg:hidden">
        ...
        <select
          ...
          id="selected-tab"
          name="selected-tab"
          className="mt-1 block w-full rounded-md border-slate-300 py-2 pl-3 pr-10 text-base capitalize focus:border-slate-400 focus:outline-none focus:ring-slate-400 dark:bg-slate-600 dark:text-white sm:text-sm"
          value={tab}
        >
          {tabItems.map((value) => (
            <option
      ...
      <div className="mb-5 hidden lg:block">
        <div className="al border-b border-slate-100">
            ...
            {tabItems.map((option) => (
              ...
              >
                <div className="flex items-center justify-center space-x-1">
                  ...
                  {option?.length ? (
                    <span className="absolute top-0 right-0 inline-flex translate-x-1/2 -translate-y-1/2 transform items-center justify-center rounded-full bg-slate-300 px-1 py-0.5 text-sm leading-none text-slate-900">
                      {option?.length}
                    </span>
                  ) : null}
                </div>
